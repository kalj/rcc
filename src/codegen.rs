use crate::{
  ...
  evaluation::{
    evaluate_const_address_expression,
    evaluate_const_int_expression,
    expression_type,
  },
  symbol_table::{
...
impl Generator {
  ...
  fn new_loop_context(&mut self, brk_lbl: &str, cnt_lbl: &str) -> LoopContext {
    std::mem::replace(&mut self.loop_ctx, LoopContext {
      break_lbl: Some(brk_lbl.to_string()),
      continue_lbl: Some(cnt_lbl.to_string()),
    })
  }
    ...
    match stmnt {
      ...
      Statement::Break(ctx) => {
        if let Some(blbl) = &self.loop_ctx.break_lbl {
          ...
          return Err(CodegenError::new("Invalid break not inside a loop or switch statement".to_string(), ctx));
        }
      },
        ...
        if let FunctionParameters::List(params) = parameters {
          for (i, p) in params.iter().enumerate() {
            ...
            if i < narg_regs {
              ...
              self
                .symtab
                .insert_local(&id, LocalVariableSymbol::new_with_stack_offset(&p.typ, stack_offset as i32));
            }
          }
        }
